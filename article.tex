...
%This makes the standard mean-squared-error evaluation to be inapplicable, at least in the original domain. 
...
%Combined, we show that Mean Squared Error in log-transformed domain can be used to evaluate speckle filters in homogeneous area.
...
The following subsection gives theoretical justification for our methodology where variance evaluation is shown to be mathematically related to the ENL index.
...
Before we begin, Fig. \ref{fig:simul_process_filter_schema} illustrates the general schema of the various experiments presented in this paper.
...
\begin{equation}
L = \frac{1}{var(\hat{X}^L) \ln^2(2)} + 0.5
...
\centering
\begin{tabular}{c|r|c|c|r}
Pattern  & AUC - true MSE (p-value) & AUC - benchmark MSE (p-value) \\
\hline
...
Pattern  & Filter  & MSE (true)   & MSE (noise)      \\% &85\%             &90\%\\
%\hline
...
\begin{tabular}{c}
	...
	\hfill	
	\subfloat[Unfiltered Noisy Image (true MSE = 4.1174)]{
		 \epsfxsize=1.5in
		 \epsfysize=1.5in
		 \epsffile{src/simulated_images.vietnam_rural.none.fi.jpg.eps} 	
		 \label{intensity}
	} \\
	\subfloat[PDE Result (true MSE = 3.8022, residual MSE = 0.0073)]{
		 \epsfxsize=1.5in
		 \epsfysize=1.5in
	...
	\hfill	
	\subfloat[Lee Result (true MSE = 0.4984, residual MSE = 3.25553)]{
		 \epsfxsize=1.5in
		 \epsfysize=1.5in
		 \epsffile{src/simulated_images.vietnam_rural.lee.fi.jpg.eps} 	
		 \label{intensity}
	} \\
	\subfloat[Frost Result (true MSE = 0.3490, residual MSE = 4.6856)]{
		 \epsfxsize=1.5in
		 \epsfysize=1.5in
	...
	\hfill	
	\subfloat[Boxcar Result (true MSE = 0.3107, residual MSE = 4.2328)]{
		 \epsfxsize=1.5in
		 \epsfysize=1.5in
...
\caption{Filtering Real Images: Visual Validation}
